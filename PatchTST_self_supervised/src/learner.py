...
class Learner(GetAttr):
    ...
    def fit(self, n_epochs, lr=None, cbs=None, do_valid=True):
        ...
        writer = SummaryWriter(f"./outputs/logs")
        ...
        try:
            for self.epoch in range(n_epochs):            
                ...
                loss_scalar_dict = {
                    'train_loss': self.recorder['train_loss'][self.epoch],
                    'valid_loss': self.recorder['valid_loss'][self.epoch],
                }
                writer.add_scalars(f"/pretrain_loss", loss_scalar_dict, self.epoch)
